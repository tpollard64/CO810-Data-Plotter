...
yaw_deg = 0     # Horizontal rotation in degrees
pitch_deg = 0  # Vertical rotation in degrees
...
img = np.roll(img, shift=-yaw_pixels, axis=1)  # negative for rightward yaw

pitch_pixels = int((pitch_deg / 180.0) * height)
img = np.roll(img, shift=-pitch_pixels, axis=0)  # negative for upward pitch
